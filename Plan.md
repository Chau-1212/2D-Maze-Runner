...
| 13/08/2026 | Evening | Fixed code bugs: `item.__init__` ignored p_effect/n_effect; raw_meat double-healed; cooked_meat/coal silently consumed; window title typo. Updated Plan.md to match actual code (Arcade not Pygame, real class structures, `data/loot_table.json`) |

---

## Current Codebase Status (13/08/2026)

- **Library:** Arcade 3.3.3 (not Pygame). `main.py` creates the window + `MainMenu` view with a click-to-start circle transition into `InGameView` (WASD movement of a test circle at 300 px/s). Note Arcade is **y-up**, so W increases y.
- **Files:** `main.py`, `player.py`, `item.py`, `chest.py`, `furance.py`, `data/loot_table.json`
- **Implemented:** menu view + transition, WASD movement, `Player` (stats, list-based inventory, `use_item`), item objects (bread/raw_meat/cooked_meat/coal/battery), `flash` (flashlight), `chest` loot generation (3 cumulative-probability rolls), `furance` class
- **Not yet implemented:** maze generation, enemy AI/BFS, wall collision, HUD, time system, loading screen, chest UI / taking items, settings & death screens
- **Known behaviour:** chest "nothing" rolls leave the slot empty, so a chest can give fewer than the rolled number of items (rolls: 2-4 by day, 3-5 at night)
...
- [ ] "Py game" / "Pygame" → **Arcade** (the game now uses Arcade, not Pygame)
- [ ] "suing" → **using**
...
- [ ] `enemies.json`: vision values → **1** (night_spider), **2** (blood_moon_spider) [rooms, not tiles] — **file doesn't exist yet**
- [ ] `loot_table.json` (at `data/loot_table.json`) matches template ✓ verified
- [ ] Items are hardcoded in `item.py` — decide: keep hardcoded, or move to `items.json` to match the JSON justification in 2c
...
- [ ] Library: **Arcade** everywhere (not Pygame) — code uses `arcade` 3.3.3
- [ ] Coordinates: Arcade is **y-up** (W increases y); plan pseudocode assumed Pygame y-down — align if pseudocode is submitted
...
- **Typos**: "stakeholder" not "skate holder", "10km" not "1000km", "Arcade" not "Pygame"/"Py game", "grace period" not "grade period"
...
- Sound effects play while game logic updates — Arcade's sound system runs on a separate audio thread
...
- **Computer:** Windows 10+, 2GHz processor, 8GB RAM, 1GB free storage
  - 2GHz dual-core minimum: Arcade is not heavily multi-threaded for game logic, so a single fast core is more important than multiple slow cores. 2D pixel-art rendering is not computationally intensive, so 2GHz is sufficient.
  - 8GB RAM: Python + Arcade use ~200MB at runtime, but the IDE (VS Code/Zed), web browser for research, and running game tests simultaneously requires more. 8GB ensures no swapping during development.
...
- **IDE:** VS Code or Zed with Python extension — chosen for the integrated debugger (essential for stepping through game logic), IntelliSense (speeds up development by auto-completing Arcade method names), and integrated terminal (quick testing without switching windows).
- **Libraries:** Arcade (graphics rendering, input handling, sound), JSON (config file parsing), random (maze generation randomness, loot drops), math (distance calculations, trig for enemy movement angles).
- **Sprite editor:** Aseprite — chosen because it is specifically designed for pixel art, has animation frame support, exports to PNG which Arcade loads natively, and has a minimal UI that doesn't distract from the art creation process.
...
- **Minimum specs:** Windows 10+, 2GHz processor, 4GB RAM, 500MB storage
  ...
  - Windows 10+: Arcade is built on pyglet/OpenGL, which provides hardware-accelerated rendering on modern Windows.
- **Software:** Python 3.10+ with Arcade installed, or a packaged executable (via PyInstaller).
...
My project uses an object-oriented modular design — separate Python classes for `Player`, `item`, `flash` (flashlight), `chest`, and `furance` (furnace), plus view classes (`MainMenu`, `InGameView`) in `main.py`. Each data structure below matches the current code exactly.

```
class Player:
    x: float                # position in pixels
    y: float
    speed: float            # 300 px/s
    health: int             # 0-100, starts at 100
    hunger: int             # 0-100, starts at 100
    lost: bool              # death flag
    win: bool               # win flag
    inventory: list         # list of [item, count] pairs, e.g. [[bread, 2]]
    selected_slot: int      # current hotbar slot (0-2)
    max_item_types: int     # 3 — max distinct item types carried
    flashlight: flash       # None until picked up; required to use batteries
    def pick_up_item(self, item, number)   # stacks by item, respects max_item_types
    def loot_chest(self, chest, num)       # NOT IMPLEMENTED yet
    def use_item(self)                     # applies effect by item_type (hunger/health/energy)
    def drop_item(self, item)
    def hot_bar(self, number)              # keys 1-3 select slot 0-2

class item:
    name: str               # "bread", "raw_meat", "cooked_meat", "coal", "battery"
    item_type: str          # "hunger", "health", "energy", "fuel"
    p_effect: int           # positive effect (hunger/health restored, battery charged)
    n_effect: int           # negative effect (future stamina penalty)

class flash:
    name: str               # "Flashlight"
    on: bool
    battery: int            # starts at 50; one spike costs 25
    vision: int             # 3 tiles normally, 5 while spiked
    spiked: bool            # scare-away active
    spike_timer: int        # counts down over 20 frames
    def turn_on(self) / turn_off(self)
    def spike(self)         # if battery >= 25: battery -= 25, vision 3→5, scares enemies

class chest:
    loot: list              # generated item objects from the loot table
    open: bool              # TRUE after first generation — loot only generated once
    def gen_loot(self, loot_table)   # 3 rolls via cumulative probability (data/loot_table.json)

class furance:              # NOTE: class/file is "furance" (furnace) in the code
    fuel: int               # +15 per coal added
    cooking: bool
    content: int            # raw meat inside
    cook_timer: float       # reaches 20s to finish cooking
    product: int            # cooked meat produced
    def add_coal(self) / add_meat(self)
    def cook(self, delta_time)
```

**External files:**
- `data/loot_table.json` — chest drop weights (exists, verified against template)
- Items are defined in `item.py` (hardcoded objects), not yet loaded from JSON
- `enemies.json` — planned for enemy stats, **not created yet** (no enemies in the code yet)

**Data structures:**
- **maze_grid:** 2D array of integers (planned — maze generation not implemented yet). Dimensions depend on room count and room size (e.g. 33×33 rooms × 5-tile rooms = ~200×200 tiles for 10km). The seed only affects which walls become doorways, not the size. Chosen over a dictionary because array indexing is O(1) and the maze is a fixed grid, making it the most efficient option for frequent collision lookups.
- **enemy:** Single Enemy object (or None) (planned — no enemies in the code yet). Only one spider exists on the map at a time — a list is unnecessary. If the player kills it or it despawns, a new one spawns later. This simplifies the code and saves memory.
- **inventory:** The code uses a **list of `[item, count]` pairs** (not the dictionary described in the design): `[[bread, 2], [coal, 1]]`. `pick_up_item` stacks by matching `item.name`, and `max_item_types = 3` limits distinct item types.

**File structures (JSON config files):** — only `data/loot_table.json` exists so far; `enemies.json` and `items.json` below are the planned formats
```
enemies.json:
{
  "enemies": [
...
loot_table.json (at `data/loot_table.json`):
{
  "normal_chest_loot": [            ← used when the chest is opened during Day
  ...
  "night_chest_loot": [             ← used at Night — better loot, rewards the Blood Moon risk
    {"item": "flashlight", "drop_rate": 10},   ← doubled (5% → 10%)
    {"item": "bread", "drop_rate": 30},
    {"item": "raw_meat", "drop_rate": 20, "quantity": "1-3"},
    {"item": "coal", "drop_rate": 20},
    {"item": "battery", "drop_rate": 10},
    {"item": "nothing", "drop_rate": 10}       ← reduced (15% → 10%) so the table still sums to 100
  ]
}
```
