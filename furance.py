...
class furance:
    def __init__(self):
        self.fuel = 0 #how much object "fuel"
        self.cooking = False
        self.content = 0
        self.cook_timer = 0#how much cooking time left
        self.product = 0
    ...
    def cook(self,delta_time):
        ...
        if self.content > 0 and self.fuel > 0: #It check before it start cooking that it have mroe than 20
            ...
            if self.cooking:
                self.cook_timer += delta_time #So it stay the same across different fps

                if self.cook_timer >= 20:
                    self.cook_timer = 0
                    self.content -= 1
                    self.product += 1 #product will be cooked meat
